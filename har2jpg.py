                ...
                if filename.endswith(".har"):
                    ...
                    with open(har_file_path, "r") as har_file:
                        ...
                        for entry in har_data["log"]["entries"]:
                            ...
                            if not (
                                "content" in response
                                and "mimeType" in response["content"]
                            ):
                                continue
                            resp_format = accepted_formats.get(
                                response["content"]["mimeType"], None
                            )
                            ...
                            if resp_format is not None:
                                ...
                                jpg_file_path = os.path.join(
                                    output_subfolder_path, jpg_filename
                                )
...
if __name__ == "__main__":
    parser = ArgumentParser(
        description="Extract JPG response data from HAR files in a folder and save it to JPG files."
    )
    parser.add_argument(
        "input_folder_path", help="Path to the folder containing HAR files"
    )
    parser.add_argument(
        "output_folder_path",
        help="Path to the output folder where JPG files will be saved",
    )
